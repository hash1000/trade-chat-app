...
router.get('/user-orders/:userId', authMiddleware, authorize(['admin','operator','user']), checkIntegerParam("userId"), checkPermission("readSingle", "orders"), orderController.getUserOrders.bind(orderController));
...
router.patch('/:orderId', authMiddleware, authorize(['admin','operator']), checkIntegerParam("orderId"), checkPermission("canUpdate", "orders"), orderController.updateOrder.bind(orderController));

router.get('/single-order/:orderId', checkIntegerParam("orderId"), authMiddleware, checkPermission("readSingle", "orders"), orderController.getOrderById.bind(orderController));

router.delete('/:orderNo', authMiddleware, authorize(['admin','operator']), checkPermission("canDelete", "orders"), orderController.deleteOrder.bind(orderController));
