...
Order.belongsTo(Address, {
  ...
  // User ↔ ProductCartItem (new product cart — separate from AddToCart/Cart)
  User.hasMany(ProductCartItem, { foreignKey: "userId", as: "productCartItems" });
  ProductCartItem.belongsTo(User, { foreignKey: "userId", as: "user" });

  ShopProduct.hasMany(ProductCartItem, { foreignKey: "shopProductId", as: "cartItems" });
  ProductCartItem.belongsTo(ShopProduct, { foreignKey: "shopProductId", as: "product" });
  ...
  // Team <-> Service many-to-many
  Team.belongsToMany(Service, {
    through: TeamServiceLink,
    foreignKey: "teamId",
    otherKey: "serviceId",
    as: "services",
  });
  Service.belongsToMany(Team, {
    through: TeamServiceLink,
  ...
  // Service <-> ServiceLike
  Service.hasMany(ServiceLike, { foreignKey: "serviceId", as: "likes" });
  ServiceLike.belongsTo(Service, { foreignKey: "serviceId", as: "service" });

  User.hasMany(ServiceLike, { foreignKey: "userId", as: "serviceLikes" });
  ServiceLike.belongsTo(User, { foreignKey: "userId", as: "user" });

  // Service <-> ServiceView
  Service.hasMany(ServiceView, { foreignKey: "serviceId", as: "views" });
  ServiceView.belongsTo(Service, { foreignKey: "serviceId", as: "service" });
  ...
  // Order <-> OrderPayment (admin-recorded payments, no wallet movement)
  Order.hasMany(OrderPayment, { foreignKey: "orderId", as: "orderPayments" });
  OrderPayment.belongsTo(Order, { foreignKey: "orderId", as: "order" });

  ServiceOrder.hasMany(OrderPayment, { foreignKey: "serviceOrderId", as: "orderPayments" });
  OrderPayment.belongsTo(ServiceOrder, { foreignKey: "serviceOrderId", as: "serviceOrder" });
