...
class UserController {
  ...
  async verify(req, res) {
    try {
      ...
      const { country_code, phoneNumber, requestId, code, email, password } = req.body

      vonage.verify.check(requestId, code)
        .then(async (resp) => {
          console.log(resp.request_id)
          // // Check if the user already exists
          let user = await userService.getUserByPhoneNumber(country_code, phoneNumber)
          let token
          if (user) {
            await userService.updateTokenVersion(user)
            // Generate JWT token
            token = jwt.sign({ userId: user.id, tokenVersion: user.tokenVersion }, process.env.JWT_SECRET_KEY)
          } else {
            // create the user and then update the password field with request id 

            // Create a new user
            user = await userService.createUser({ email: email, password: password, phoneNumber, country_code: country_code })
            // Generate JWT token
            token = jwt.sign({ userId: user.id, tokenVersion: 0 }, process.env.JWT_SECRET_KEY)
          }
          // Respond with the token and user data
  ...
  async changePassword(req, res) {
    try {
      ...
      const updateduser = await userService.updateUserPassword(user.id, newPassword)
      const token = jwt.sign({ userId: updateduser.id, tokenVersion: updateduser.tokenVersion }, process.env.JWT_SECRET_KEY)
      // Respond with the token and user data
  ...
  async createAdmin(req, res) {
    try {
      ...
      const updatedUser = await userService.updateUserProfile(user, { role: 'admin' })
  ...
  async getUsers(req, res) {
    try {
      ...
      const products = await userService.getPaginatedUsers(page, limit, search, req.user.id)
  ...
  async resetPassword(req, res) {
    ...
    try {
      ...
      if (userId) {
        ...
        const token = jwt.sign({ userId: user.id, tokenVersion: user.tokenVersion }, process.env.JWT_SECRET_KEY)
        // Respond with the token and user data
  ...
  async updateUser(req, res) {
    try {
      ...
      const updatedUser = await userService.updateUserProfile(user, { name, profilePic, settings })
  ...
  async verifyEmailOrPhone(req, res) {
    try {
      ...
      if (phone) {
        ...
        const userByPhone = await userService.getUserByPhoneNumber(country_code, phoneNumber)
        if (userByPhone) {
        ...
        if (userByEmail) {
          return res.status(409).json({ message: 'Email already in use', error: err })
        }
  ...
  async makePrimary(req, res) {
    try {
      ...
      if (email) {
        ...
        const { otp: userByEmailCode } = await userService.getUserOTPCode(user.id)
        if (!userByEmailCode) {
        ...
        vonage.verify.check(request_id, otp)
          .then(async (resp) => {
            await userService.updateUserProfileById(user.id, {country_code, phoneNumber})
            const updatedUser = await userService.getUserById(user.id)
            res.json({ updatedUser })
          })
          .catch(err => {
