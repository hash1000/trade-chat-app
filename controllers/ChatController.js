...
class ChatController {
  ...
  async getMessages (req, res) {
    ...
    const message = await chatService.getMessages(chatId, page, pageSize, messageId, userId)
    res.json(message)
  }
  ...
  async getUserTransactions (req, res) {
    ...
    const transactions = await chatService.getUserTransactions(userId, specificUserId, finalFrom, finalTo)
  ...
  async sendPaymentRequest (req, res) {
    ...
    const paymentRequest = await chatService.sendPaymentRequest(Number(requesterId), Number(requesteeId), amount, req.user, req)
  ...
  async sendPayment (req, res) {
    ...
    const payment = await chatService.sendPayment(Number(requesterId), Number(requesteeId), amount, req.user, req)
    res.json(payment)
  }

  async bulkForwardMessages (req, res) {
    const { body: {payload, recipientId }, files }= req;
    const { id: userId } = req.user
    const messages = await chatService.bulkForwardMessages(payload, files, userId, recipientId, req.user, req)
    res.json({
