...
class UserRepository {
  ...
  async getUserProfile(id) {
    const user = await  User.findOne({ where: 
      { id }, 
      attributes: ['id','firstName','lastName','username', 'role', 'email', 'phoneNumber', 'country_code', 'profilePic','settings', 'createdAt', 'updatedAt', 'likes', 'dislikes', 'last_login']
    })
  ...
  async updateUserPassword (email, newPassword) {
    ...
    await User.update({ password: newPassword, resetToken: null }, { where: { email } })
  }
  ...
  async getUsersByPhoneNumbers (phoneNumbers) {
    const phoneNumbersArray = phoneNumbers.map(phoneNumber => phoneNumber.phone);
    ...
    users = users.filter(user => {
      const userPhoneNumber = { country: user.country_code, phone: user.phoneNumber };
