...
const { PaymentReceivedNotification, PaymentRequestNotification, NewMessageNotification }=require('../notifications')
...
class CartService {
  ...
  async getMessages (chatId, page, pageSize, messageId, userId) {
    return await this.chatRepository.getMessages(chatId, page, pageSize, messageId, userId)
  }
  ...
  async getUserTransactions (userId, specificUserId, from, to) {
    return await this.chatRepository.getUserTransactions(userId, specificUserId, from, to)
  }
  ...
  async sendPayment (requesterId, requesteeId, amount, user, req) {
    ...
    const paymentRequest = await this.paymentRepository.createPaymentRequest(requesterId, requesteeId, amount, 'accepted')

    // create a message
    const message = await this.chatRepository.createMessage(chatId, requesterId, `Payment of ${amount}`, paymentRequest.id)
  ...
  async bulkForwardMessages (payload, files, userId, recipientId, user, req) {
    // get chat id if chat exists else create chat
    const chat = await this.chatRepository.findOrCreateChat(userId, recipientId)
    const { id: chatId } = chat;
    ...
    fileUrls = await Promise.all(files.map(async file => {
      ...
      const fileName = `${chatId}-${Date.now()}${path.extname(file.originalname)}`;
      const params = {
    ...
    // create messages
    const messages = await this.chatRepository.createMessages(chatId, userId, modifiedPayload)
