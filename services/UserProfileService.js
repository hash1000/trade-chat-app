...
const { AddRequestNotification, RequestAcceptedNotification } = require('../notifications')
...
class UserService {
  async getUserProfileById (profileId, userId) {
    const my_reaction = await reactionRepository.getReactions(userId, profileId)
    ...
    const disliked = (my_reaction && my_reaction.type === 'dislike') ? true : false
    ...
    try {
      ...
      if (my_reaction) {
        if (my_reaction.type !== type) {
          ...
          await reactionRepository.updateReaction(userId, profileId, type, transaction)
          if (type === 'like') {
            await userRepository.updateReactionCount(profileId, 'dislikes', dislikeCount - 1, transaction)
            await userRepository.updateReactionCount(profileId, 'likes', likeCount + 1, transaction)
        ...
        await reactionRepository.createReaction(userId, profileId, type, transaction)
        await userRepository.updateReactionCount(profileId, `${type}s`, reactionCount + 1, transaction)
      }
  ...
  async removeReaction(userId, profileId, type) {
    ...
    try {
      const user = await userRepository.getUserCounts(profileId)
      const updated = await reactionRepository.removeReaction(userId, profileId, type, transaction)
      if (updated > 0) {
        const reactionCount = user[`${type}s`] || 0
        await userRepository.updateReactionCount(profileId, `${type}s`, reactionCount - 1, transaction)
      }
    ...
    if (friendship) {
      if (friendship.type === 'received') {
        ...
        if (otherUser && otherUser.fcm && myUser && myUser.name) {
          await new RequestAcceptedNotification(otherUser.fcm,{},myUser).sendNotification();
        }
      }
    } else {
      ...
      if (otherUser && otherUser.fcm && myUser && myUser.name) {
        await new AddRequestNotification(otherUser.fcm,{},myUser).sendNotification();
      }
    }
  }
  ...
  async getUserContacts(userId) {
    ...
    const accepted = friends.filter(friend => friend.friendship.type === 'accepted')
    const pending = friends.filter(friend => friend.friendship.type === 'sent')
    const received = friends.filter(friend => friend.friendship.type === 'received')
